...
export interface ShopifyClient {
  ...
  readonly products: {
    list(args: { first: number; cursor?: string; query?: string }): Promise<{
  ...
  readonly collections: {
    list(args: { first: number; cursor?: string }): Promise<{
  ...
  readonly customers: {
    list(args: { first: number; cursor?: string; query?: string }): Promise<{
      items: ShopifyCustomer[];
      nextCursor: string | null;
    }>;
    read(args: { id: string }): Promise<ShopifyCustomer>;
  };
  readonly inventory: {
    read(args: { locationId: string; inventoryItemIds: string[] }): Promise<
      ShopifyInventoryLevel[]
    >;
  };
}
